...
registerBlock(0, 32, 0, 1, function(x, y)
	donutStates[x + y * 32] = false
end, function(x, y)
	if donutStates[x + y * 32] then
		setBlockEnabled(x, y, false)
	end
	if player.getX() == x and player.getY() == y then
		donutStates[x + y * 32] = true
	end
...
for i=0,127 do
	...
	registerBlockImportant(32, 0, 128 + i, 1, function() end, function(x, y)
		if player.getX() == x and player.getY() == y then
			switches[i] = true
		end
		if player.getState() == 1 and player.getX() + 1 == x and player.getY() == y then
	...
	registerBlockImportant(32, 32, 128 + i, 1, function() end, function(x, y)
		if player.getX() == x and player.getY() == y then
	...
	registerBlockImportant(32, 64, 128 + i, 1, function() end, function(x, y)
		if player.getX() == x and player.getY() == y and player.getState() == 0 then
			switches[i] = true
		end
	end, function(x, y)
	...
	registerBlockImportant(32, 96, 128 + i, 1, function() end, function(x, y)
		if player.getX() == x and player.getY() == y and player.getState() == 0 then
			switches[i] = not switches[i]
		end
	end, function(x, y)
